...
for log_handler in numba_loggers:
    log_handler.setLevel(logging.INFO)
...
if __name__ == "__main__":
    with open(os.path.join("Experiment", "train_director_burn.yaml"), "r") as stream:
        try:
    ...
    model_path = train(
        ...
        steps=100, 
        seed=0, 
        model_dir="burn_director"
        )

    # model_path = min(glob.iglob(os.path.join('director', '*.zip')), key=os.path.getctime)
    rew = eval(
        get_env_pz,
    ...
    with open(os.path.join("Experiment", "dash_app_template_burn.yaml"), 'r') as f:
